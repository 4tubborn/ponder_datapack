...
#block:<string>，格式为<block_id>*[<block_state>]*{block_entity}，即与setblock ~ ~ ~ 后的内容相同。show_particle:<bool>，是否显示破坏粒子
...
execute if data storage ponder:scene {cur_cmd:{type:"modify_tile_nbt"}} run return run function ponder:process/scene/single/cmd/modify_tile_nbt/ with storage ponder:scene cur_cmd
...
#x:<int>,y:<int>,z:<int> 锚点,*scale:[3]缩放，默认为[1,1,1], *duration:<int>默认为20,*color:<int>默认为-1（白色），覆盖发光边框颜色，
...
#selector:<目标选择器>,operation:<string>（与data modify block ~ ~ ~ 后的内容相同）
